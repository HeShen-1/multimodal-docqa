...
面向春招 AI 应用 / RAG 后端岗位的多模态文档问答项目，当前重点展示：

- 稳定主链路：上传 → 处理 → 检索 → 流式回答 → 分享访问
- 工程可信度：鉴权、权限、健康检查、真实统计、测试与 CI
- AI 含金量：评测集、检索对比实验模板、Prompt 版本化、拒答策略
- 展示完整度：根 README、架构图、Demo 路径、简历与面试材料

> 当前已知前提：`Ollama` 连接暂不作为阻塞项，后续可替换为你的微调模型；本轮工程链路和评测材料已为后续模型接入预留接口。
